...
class TextDataset:
    ...
    def _get_feats(self, args, base_attrs):
        ...
        # Use the exact same filtered rows as labels and audio/video features.
        train_examples = [InputExample(index, text) for index, text in
                          zip(base_attrs['train_data_index'], base_attrs['train_data_text'])]
        train_feats = self._get_bert_feats(args, train_examples, base_attrs)

        dev_examples = [InputExample(index, text) for index, text in
                        zip(base_attrs['dev_data_index'], base_attrs['dev_data_text'])]
        dev_feats = self._get_bert_feats(args, dev_examples, base_attrs)

        test_examples = [InputExample(index, text) for index, text in
                         zip(base_attrs['test_data_index'], base_attrs['test_data_text'])]
    ...
    def _get_bert_feats(self, args, examples, base_attrs):
        ...
        if args.text_backbone.startswith('bert'):
            tokenizer = BertTokenizer.from_pretrained('/public/home/202420144954/bert-large-uncased', do_lower_case=True)   
